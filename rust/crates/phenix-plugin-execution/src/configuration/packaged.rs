...
fn reconcile<T: Clone + Eq>(
    ...
    for (id, expected) in owned.iter() {
        if records.get(id) != Some(expected) {
            return Err(format!(
                "packaged callable changed outside configuration: {id}"
            ));
        }
    }
...
fn unique<T>(
    values: Vec<T>,
    id: impl Fn(&T) -> &CallableId,
) -> Result<BTreeMap<CallableId, T>, String> {
...
pub(super) fn configure(
    ...
    state.packaged.active_agents =
        reconcile(&mut state.agents, &mut state.packaged.agents, agents)?;
    let available_agents = state
        .agents
        .iter()
        .filter(|(id, _)| {
            !state.packaged.agents.contains_key(*id) || state.packaged.active_agents.contains(*id)
        })
    ...
    state.packaged.active_orchestrations = reconcile(
        &mut state.orchestrations,
        &mut state.packaged.orchestrations,
        orchestrations,
    )?;
    let prepared: ModelResponse = context
        .sdk
        .invoke_projected(&ModelCommand::PreparePackagedProfiles { profiles })
    ...
    context
        .kernel
        .transact_prepared(&[mutation, execution])
        .map_err(|error| error.to_string())?;
