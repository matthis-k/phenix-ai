...
fn temp_db() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "phenix-root-reservation-{}-{nonce}.sqlite",
        std::process::id()
    ))
}
...
fn reserve_then_settle_releases_only_unused_root_step_capacity() {
    ...
    let ExecutionResourceResponse::Remaining { budget } = reserved else {
        panic!("expected remaining")
    };
    ...
    let ExecutionResourceResponse::Remaining { budget } = settled else {
        panic!("expected remaining")
    };
