...
use phenix_sdk::{
    ...
    ContextResponse, ContextScope, ExactContextReference, ExecutionCommand, ExecutionContextProjection,
...
fn context<'host, 'runtime>(host: &'host PluginHost<'runtime>) -> ContextPluginContext<'host, 'runtime> {
    PluginContext::new(
        host,
        ContextSdk {
...
fn handle(
    ...
    match command {
        ...
        ContextCommand::Load {
            ...
            Ok(ContextResponse::Loaded { injection, resource })
        }
...
fn register_resource(
    ...
    if let Some(existing) = read_raw(context, &key)? {
        let existing: ContextResourceRevision = serde_json::from_slice(&existing)
            .map_err(|error| error.to_string())?;
    ...
    refs.sort();
    refs.dedup();
    context.kernel.transact_durable(
        &context_namespace(),
        &[
            TransactionOp::AssertValue { key: key.clone(), expected: None },
            TransactionOp::AssertValue { key: ALL_RESOURCES_KEY.into(), expected: old_refs },
            TransactionOp::Put {
                key,
...
fn discover_repository(
    ...
    for source in sources {
        let Some(kind) = project_file_kind(&source.path) else { continue; };
        ...
        let resource_id = ContextResourceId::parse(format!("{prefix}:{workspace_id}:{}", source.path))
            .map_err(str::to_owned)?;
        let resource = register_resource(context, resource_id, kind, source.path, scope, source.content)?;
        descriptors.push(resource.descriptor);
    }
    descriptors.sort_by(|left, right| left.resource_id.cmp(&right.resource_id).then_with(|| left.revision.cmp(&right.revision)));
    Ok(descriptors)
}
...
fn load_context(
    ...
    let sequence = u64::try_from(injections.len())
        .map_err(|_| "context injection sequence overflow".to_owned())? + 1;
    ...
    let next_state_bytes = state_changed
        .then(|| next_state.snapshot().map_err(|error| format!("context state snapshot failed: {error:?}")))
        .transpose()?;

    let mut operations = vec![
        TransactionOp::AssertValue { key: key.clone(), expected: old_injections },
        TransactionOp::Put {
            key,
    ...
    context.kernel.transact_durable(&context_namespace(), &operations)
        .map_err(|error| error.to_string())?;
    if state_changed {
        *state = next_state;
    }
...
fn require_active_execution(
    ...
    let response = context.sdk.execution
        .invoke_projected::<ExecutionCommand, ExecutionResponse>(&ExecutionCommand::GetExecution {
            id: execution_id.to_owned(),
        })
        .map_err(|error| error.to_string())?;
    match response {
        ExecutionResponse::ExecutionLookup { execution: Some(execution) }
            if execution.state == ExecutionState::Active => Ok(()),
        ExecutionResponse::ExecutionLookup { execution: Some(execution) } => Err(format!(
            "context target execution is not active: {execution_id} ({:?})", execution.state
        )),
        ExecutionResponse::ExecutionLookup { execution: None } => Err(format!("unknown context target execution: {execution_id}")),
...
fn project_context(
    ...
    validate_identity("execution id", &execution_id)?;
    let injections = decode_injections(read_raw(context, &injections_key(&execution_id))?.as_deref())?;
    let entries = injections.into_iter().map(|injection| {
        let resource = read_resource(context, &injection.source.resource_id, &injection.source.revision)?
            .ok_or_else(|| format!("missing durable context revision: {}@{}", injection.source.resource_id, injection.source.revision))?;
...
fn list_descriptors(context: &ContextPluginContext<'_, '_>) -> Result<Vec<ContextDescriptor>, String> {
    let refs = decode_refs(read_raw(context, ALL_RESOURCES_KEY)?.as_deref())?;
    refs.into_iter().map(|reference| {
        read_resource(context, &reference.resource_id, &reference.revision)?
            .map(|resource| resource.descriptor)
            .ok_or_else(|| format!("missing durable context revision: {}@{}", reference.resource_id, reference.revision))
    }).collect()
}

fn read_raw(context: &ContextPluginContext<'_, '_>, key: &str) -> Result<Option<Vec<u8>>, String> {
    context.kernel.read_durable(&context_namespace(), key).map_err(|error| error.to_string())
}

fn decode_refs(value: Option<&[u8]>) -> Result<Vec<ExactContextReference>, String> {
    value.map(|value| serde_json::from_slice(value).map_err(|error| error.to_string()))
...
fn validate_identity(label: &str, value: &str) -> Result<(), String> {
    if value.trim().is_empty() { Err(format!("{label} must not be empty")) } else { Ok(()) }
}
