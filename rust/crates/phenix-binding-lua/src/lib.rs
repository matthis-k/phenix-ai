...
use phenix_core::{
    ...
    ObjectRef, PhenixSchema, PhenixValue, PluginId, ReferenceId, RoutingProfileId, Type, ValueCodec,
};
use std::{
    cell::RefCell,
...
fn application_selection_info(
    ...
    Ok(SelectionInfo {
        provider: option.meta.as_ref()
            .and_then(|meta| meta.get("phenix.provider"))
            .and_then(serde_json::Value::as_str)
            .ok_or_else(|| BindingError::conversion("Phenix routing option is missing provider metadata"))
            .and_then(|provider| PluginId::parse(provider).map_err(|error| BindingError::conversion(error.to_string())))?,
...
mod tests {
    ...
    fn standard_acp_model_config_projects_to_application_selections() {
        let provider = serde_json::Map::from_iter([("phenix.provider".into(), serde_json::json!("openai-codex"))]);
        let options = vec![SessionConfigOption::select(
            ...
            vec![
                SessionConfigSelectOption::new("router.balanced", "[router] Balanced")
                    .description("Adaptive route").meta(provider.clone()),
                SessionConfigSelectOption::new("model.openai-codex.gpt-5", "[model] GPT-5")
                    .description("Fixed model").meta(provider),
            ],
        )];
