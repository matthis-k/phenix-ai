...
impl StopView<'_> {
    fn stop(&self, plugin: &PluginId, instance: &Arc<Mutex<Box<dyn PluginInstance>>>) {
        ...
        let host = PluginHost {
            ...
            plugin,
            scope: CallScope::root(plugin, &manifest.maximum_authority, Some(live_call.cancellation_token().clone())),
            events: self.events,
            tasks: self.tasks,
            ...
            for plugin in candidate_config.activation_order() {
                    ...
                    match &manifest.execution {
                        ...
                        PluginExecution::Runtime { runtime, artifact } => {
                            ...
                            let host = PluginHost {
                                ...
                                scope: CallScope::root(&binding.provider, &provider_manifest.maximum_authority, Some(cancellation.clone())),
                                events: &self.events,
                                tasks: &self.tasks,
                                persistence: &self.persistence,
                                prepared_mutations: &prepared_mutations,
                ...
                if let Some(mut instance) = instance {
                    ...
                    let host = PluginHost {
                        ...
                        instances: &next_instances,
                        plugin,
                        scope: CallScope::root(plugin, &manifest.maximum_authority, Some(cancellation.clone())),
                        events: &self.events,
                        tasks: &self.tasks,
                        persistence: &self.persistence,
                        prepared_mutations: &prepared_mutations,
