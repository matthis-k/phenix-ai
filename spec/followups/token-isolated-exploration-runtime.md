...
5. Schedule the ordinary worker only after durable admission. Before start, a confirmed schedule failure cancels the task and releases its unused reservation atomically; add that operation to the execution owner if absent. After dispatch or an uncertain outcome, settle known use or retain conservative reservation charges. Never release spent/unknown usage as zero.
